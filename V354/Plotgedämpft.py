...
dU, dt = np.genfromtxt ("Messwerte.txt",unpack=True)
#U=np.log(dU)
dt=dt*10**-6
...
xneu=np.linspace(min(dt),max(dt),100)
params, covariance = curve_fit(f, dt, dU)
errors = np.sqrt(np.diag(covariance))
print ("A=", params[0] ,"+-", errors[0])
print ("b=", params[1] ,"+-", errors[1])

plt.plot(xneu, f(xneu, *params), 'b-', label='Ausgleichskurve', linewidth=1)
plt.legend(loc="best")
