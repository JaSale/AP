...
x, y = np.genfromtxt('d1.txt', unpack=True)
...
yposition = [45, 135]
for yc in yposition:
    plt.axhline(y=yc, color='g', linestyle=':', label=r"$±\nu$")

xposition = [31, 44.5]
for xc in xposition:
    plt.axvline(x=xc, color='y', linestyle=':', label=r"$\frac{\pi}{4}, \frac{3\pi}{4}$")
L1plot = np.linspace(0, 0)#Grenzen fuer linregr. guckt nach den Grenzwerten fuer die x-Werte.
plt.xlim(14, 65)
plt.ylim(0, 205)
plt.title('Frequenzabhängige Phasenverschiebung') #Ueberschrift, die ueber dem PLot stehen wird
...
#Achsen beschriften!!!!!!
plt.ylabel(r"$\Delta \phi / °$") #hier wurde Tex Code verwendet, um Brueche ordentlich darzustellen. das T_P macht, dass das P als Indesx von T erscheint. Hinter den Slash kommt die Einheit fuer die jeweilige Groesse
plt.xlabel(r"$ \nu / \mathrm{kHz}$")
...
#plt.show()
plt.savefig('plotD2.pdf') #erstellt und speichert automatisch den Plot als pdf datei
